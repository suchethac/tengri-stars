...
def _toy_isochrones(tmp_path):
    ...
    for feh, dt in ((-2.5, 0.0), (-1.0, -120.0)):
        ...
        for i, teff in enumerate(np.linspace(4000.0 + dt, 6300.0 + dt, 30)):
            rows.append([i, 0.7, np.log10(teff), 5.0 - 0.015 * i, 0.0, 0.0,
                         7.5 - 0.05 * i, 0.0, 0.0, 0.0, 0.0])
        # RGB: teff descends from the turnoff, logg 3.5 -> 0.6, bright M_g
        for i, teff in enumerate(np.linspace(6200.0 + dt, 4100.0 + dt, 30)):
            rows.append([30 + i, 0.8, np.log10(teff), 3.5 - 0.1 * i, 0.0, 0.0,
                         3.0 - 0.12 * i, 0.0, 0.0, 0.0, 0.0])
...
def env(tmp_path):
    ...
    tables = load_dartmouth_branches(
        _toy_isochrones(tmp_path), model, ig=IG, ii=II,
        gi_grid=np.arange(0.2, 3.2, 0.005), feh_step=0.05, rgb_logg_max=3.6,
    )
...
def test_known_dm_prefers_true_branch(env):
    ...
    scans = binary_scan(obs, SIGMA, tables, model, ig=IG, ii=II,
                        chi2_bands=CHI2_BANDS, dm_known=16.0)
...
def test_mixture_equal_curves_gives_prior(env):
    ...
    scans = {"RGB": dict(flat), "MS": {"chi2_curve": np.zeros((n_scan, 1)),
                                       "mg": np.full((n_scan, 1), 7.0)}}
...
def test_mixture_parallax_breaks_tie(env):
    ...
    out = combine_mixture(scans, g_obs, tables.feh_scan,
                          parallax=np.array([1.0]), parallax_error=np.array([0.1]))
    assert out["p_rgb"][0] < 1e-6  # parallax says nearby -> the dwarf hypothesis
    out2 = combine_mixture(scans, g_obs, tables.feh_scan,
                           parallax=np.array([0.06]), parallax_error=np.array([0.02]))
...
def test_real_grid_noiseless_closure():
    ...
    model = StarModel(grid=grid, interp_method="pchip")
    files = {feh: str(REAL_ISO / name) for feh, name in
             ((-2.5, "fehm25_12Gyr.txt"), (-2.0, "fehm20_12Gyr.txt"),
    ...
    for branch in ("RGB", "MS"):
        for k_node, gi0 in ((1, 0.9), (3, 1.1)):
            ...
            scans = binary_scan(obs, sigma, tables, model, ig=1, ii=3,
                                chi2_bands=(0, 1, 2, 3))
